...
BEGIN
	...
	(
		...
		previusUnitCost  decimal(20,6),
		previusCostTotal  decimal(14,6) 
	)
	...
	(
		...
		unitPriceMove decimal(14,6),-- Cost Unitario Cantidad --AmountCotsUnit
		unitCostTotal decimal(14,6), -- Costo totalCantidad - AmountCostTotal
		previusUnitCost  decimal(20,6), -- Costo Unitario Previo
		previusCostTotal  decimal(20,6) -- Costo Total Previo
	)
	...
	--select * from #TmpIdentificadoresInventoryMoveDetailReduced
	--where id_item = 2601
	...
	(
		...
		dateCreate datetime,
		-- Cost
		previousUnitCost decimal(20,6), -- Costo Unitario
		previousCostTotal decimal(20,6), -- Costo totalCantidad - AmountCostTotal
	)
	...
	SELECT	A.id, 
			...
			NULL,
			--0
			SUM(ISNULL(AA.unitPriceMove, 0) ) as previousUnitCost,
			SUM( (ISNULL(AA.entryAmount, 0) - ISNULL(AA.exitAmount, 0)) * ISNULL(AA.unitPriceMove, 0)   ) as previousCostTotal
	...
	BEGIN
		...
		FROM #TmpIdentificadoresInventoryMoveDetail A
		--WHERE (A.codeDocumentState = '03') AND (A.emissionDate < DATEADD(day, 1, @startEmissionDate) OR @startEmissionDate IS NULL)
		WHERE (A.codeDocumentState = '03') AND ( (A.emissionDate <   @startEmissionDate)  OR @startEmissionDate IS NULL)

		--DELETE A
		--FROM #TmpInventoryMoveDetailToNoShow A WITH (NOLOCK)
		--INNER JOIN #TmpIdentificadoresInventoryMoveDetailReduced AA WITH (NOLOCK) 
		--ON ( AA.id_item = A.id_item 
		--	 AND AA.id_warehouse = A.id_warehouse
		--	 AND AA.id_warehouseLocation = A.id_warehouseLocation
		--	 AND (ISNULL(AA.id_lot, 0) = ISNULL(A.id_lot, 0)))
		--WHERE (AA.codeDocumentState = '03')  

				--/*AND AA.id_warehouseLocation = A.id_warehouseLocation AND (ISNULL(AA.id_lot, 0) = ISNULL(A.id_lot, 0))*/ )
		--select * from #TmpInventoryMoveDetailToNoShow
		--where id_item = 2601
		...
		SELECT	Max(A.id), 
				...
				A.id_item, 
				A.id_warehouse, 
				max(A.id_warehouseLocation) id_warehouseLocation,  
				max(A.id_lot) id_lot,
	...
	SET A.previousBalance = ISNULL(AA.previousBalance, 0),
		...
		A.previusCostTotal = ISNULL(AA.previousCostTotal, 0),
		A.previusUnitCost = ISNULL(AA.previousUnitCost,0)
	...
	SELECT 
			...
			,cast(x.emissionDate as Date) [fechaEmison]
			...
			CASE 
				WHEN ISNULL(x.id_inventoryReason,0) = 0 
						then (CASE 
								WHEN x.previousTotalCostPound = 0 or x.previousPound = 0 THEN 0
			...
			x.entradaPound,
			CASE 
				WHEN x.entradaTotalCostPound = 0 or x.entradaPound=0 THEN 0
			...
			x.salidaPound,
			CASE 
				WHEN x.salidaTotalCostPound = 0 or x.salidaPound = 0 THEN 0
			...
			x.finalPound,
			CASE 
				WHEN x.finalTotalCostPound = 0 or x.finalPound=0 THEN 0
			...
			x.[previusCostTotal],
			x.[previusUnitCost],
			(x.previousTotalCostPound +x.entradaTotalCostPound) -x.salidaTotalCostPound [finalTOTAL],
			x.masterCode
	FROM
	(
		SELECT 
		--------------- CAMPO ORIGINALES
			...
			--- Saldo Inicial
			CASE 
			  WHEN Y.factor IS NULL THEN ( round(x.previousBalance * x.ItemPresentationValue ,2)) 
			  ELSE ( round(x.previousBalance * x.ItemPresentationValue * Y.factor,2)) 
			END AS [previousPound],
			(x.previousBalance * x.amountCostUnit) [previousTotalCostPound],
			-------------------
			--- Ingresos
			CASE 
			  WHEN Y.factor IS NULL THEN ( round(x.[entry] * x.ItemPresentationValue,2) ) 
			  ELSE ( round(x.[entry] * x.ItemPresentationValue * Y.factor,2)) 
			END AS [entradaPound],
			( x.[entry] * x.amountCostUnit) [entradaTotalCostPound],
			-------------------
			--- Egresos
			CASE 
			  WHEN Y.factor IS NULL THEN (round( x.[exit]  * x.ItemPresentationValue ,2)) 
			  ELSE ( round( x.[exit] * x.ItemPresentationValue * Y.factor,2)) 
			END AS [salidaPound],
			(x.[exit] * x.amountCostUnit) [salidaTotalCostPound],
			-------------------
			--- Saldo Final
			CASE 
			  WHEN Y.factor IS NULL THEN ( round(x.balance * x.ItemPresentationValue,2) ) 
			  ELSE ( round( x.balance * x.ItemPresentationValue * Y.factor ,2)) 
			END AS [finalPound],
		...
		(
			SELECT  A.id
				   ,A.id_inventoryMove id_document
					...
					ISNULL(B.previusCostTotal,0) [previusCostTotal],
					ISNULL(B.previusUnitCost,0) [previusUnitCost],
					G.masterCode
				...
				INNER JOIN dbo.InventoryLine IVL WITH (NOLOCK) ON (G.id_inventoryLine = IVL.id )
				...
				(SELECT AA.id
						...
						case 
						  when ISNULL(AB.previounitCostTotal,0) = 0 or ISNULL(AB.previousBalance,0) =0 then 0
						  else
							(ISNULL(AB.previounitCostTotal,0) /  ISNULL(AB.previousBalance,0)) 
						end as [amountCostUnit],
				...
				INNER JOIN dbo.InventoryLine IVL WITH (NOLOCK) ON (AG.id_inventoryLine = IVL.id )
			  ...
			  and Y.id_metricDestiny = @UnidadMedidaLbsId
		) X 
		--where X.code_item LIKE '%PT000378%'   
		--order by X.[id_warehouse],
		--			 X.[id_item],
		--			 X.[emissionDate],
		--			 X.[documentType] desc, -- nombreMotivoInventario
		--			 X.[id_document],
		--			 X.id
		END
		ELSE
...
/*
-- Pruebas:			
EXEC [dbo].[inv_Consultar_Kardex_Saldo_Costo_StoredProcedure] '{"id_documentType":0,"number":"Todos","reference":"Todas","startEmissionDate":"2021-09-01T00:00:00","endEmissionDate":"2021-09-30T00:00:00","idNatureMove":0,"id_inventoryReason":0,"id_warehouseExit":0,"id_warehouseLocationExit":0,"id_dispatcher":0,"id_warehouseEntry":29,"id_warehouseLocationEntry":0,"id_receiver":0,"numberLot":"Todos","internalNumberLot":"Todos","items":"Todos","id_user":0,"codeReport":"KEDST"}'
*/