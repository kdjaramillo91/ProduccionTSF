/****** Object:  StoredProcedure [dbo].[spPar_LibrasLiquidadasPendientePagoPAprobacion]    Script Date: 15/06/2023 04:49:43 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- SP Tomado Originalmente de: par_ProductionPoundsliquidationPayPendientPAprobacion
CREATE OR ALTER   PROC [dbo].[spPar_LibrasLiquidadasPendientePagoPAprobacion]
@id varchar(50) = '',
@fi varchar(10) = '',
@ff varchar(10) = '',
@proveedor int = 0
...
select 
	   ...
	   Company.phoneNumber as telephone_cia,
		...
		CASE WHEN YEAR(ISNULL(@fiDt, '')) = 1900 THEN 'TODOS' ELSE CONVERT(VARCHAR(10), @fiDt, 103) END AS Fi,
		CASE WHEN YEAR(ISNULL(@ffDt, '')) = 1900 THEN 'TODOS' ELSE CONVERT(VARCHAR(10), @ffDt, 103) END AS Ff
...
WHERE pl.internalNumber like case when @id = '' then pl.internalNumber else '%' + @id + '%' end
...
/*
	EXEC spPar_LibrasLiquidadasPendientePagoPAprobacion '', '2019/03/01', '2019/04/30', 0
*/
GO