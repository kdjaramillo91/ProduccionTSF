/****** Object:  StoredProcedure [dbo].[spPar_LiquidacionCompraCamaron]    Script Date: 11/05/2023 02:24:24 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- Query original: par_ProductionLiquidationPreliminaryCR
CREATE OR ALTER PROC [dbo].[spPar_LiquidacionCompraCamaron]
(
	@id int  
)
As   
...
select 
   PLP.[id],  
	...
	dbo.FUN_GetRemissionGuide(PLP.id_productionLot) as [GuiaRemision],  
 ...
 LIBRASREMITIDAS =(select SUM(quantityRecived) from ProductionLotDetail where id_productionLot= @id)  ,
  --case when ptt.code = 'COL' and mup.code = 'Kg' THEN PLL.quantity * (2.2046 * pre.minimum)
  --else PLL.quantityTotal end  AS [CANTIDAD],---------------------------EV  
  PLP.totalPounds AS [CANTIDAD],
...
inner join @DatosLiquidacion PLL on PLL.[id_item] = PLP.[id_item] and PLP.id_productionLot = PLL.id_productionLot
...
inner join [dbo].[Item] Item  on Item.[id] = PLP.[id_item]  
...
inner join [dbo].[MetricUnit] MU on MU.id = PLP.id_metricUnitProcess  
...
inner join [dbo].[Document] Document on Document.id = PLP.id_productionLot  
...
/*
	EXEC [dbo].[spPar_LiquidacionCompraCamaron] 731580
	EXEC spPar_LiquidacionCompraCamaron @id=942030
*/
GO