/****** Object:  StoredProcedure [dbo].[spPar_MovimientoIngresoMasterizado]    Script Date: 26/01/2023 16:02:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROC [dbo].[spPar_MovimientoIngresoMasterizado] 
	@id int
as 
set nocount on 
...
,RazonEgreso = ISNULL((select ir.name from documentsource ds
inner join document d on d.id = ds.id_documentOrigin
left join document d2 on d2.id = ds.id_document
inner join inventorymove im on im.id = ds.id_document
--and im.idNatureMove = 5 and d.id_documentType = 1166
...
where id_documentOrigin = @id
And d2.id_documentState <> 5), '')
,SecEgreso = ISNULL((select d2.number from documentsource ds
inner join document d on d.id = ds.id_documentOrigin
left join document d2 on d2.id = ds.id_document
inner join inventorymove im on im.id = ds.id_document
--and im.idNatureMove = 5 and d.id_documentType = 1166
...
Isnull(lot.internalNumber,pl.internalNumber) else Isnull(lot.internalNumber,pl.internalNumber) end
,com.logo
,com.logo2
,isnull(Lot.internalNumber, pro.internalNumber) as LoteMP
from Mastered Mastered
inner join Document Document on Document.id = Mastered.id --DOCUMENTO
inner join EmissionPoint emi on Document.id_emissionPoint=emi.id
...
left  join ProductionLot Pro on Pro.id = MasteredDetail.id_lotMP
Inner Join ProductionLot pl On pl.id = lot.id
...
/*
	EXEC spPar_MovimientoIngresoMasterizado 708843
*/
