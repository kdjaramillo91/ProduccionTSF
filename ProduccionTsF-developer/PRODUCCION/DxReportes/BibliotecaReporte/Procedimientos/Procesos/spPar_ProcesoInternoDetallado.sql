/****** Object:  StoredProcedure [dbo].[spPar_ProcesoInternoDetallado]    Script Date: 25/04/2023 11:34:45 a. m. ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   PROC [dbo].[spPar_ProcesoInternoDetallado]
(
--LOTE
@id varchar(50) ='',
@estado int =0,
...
set @fiev=CONVERT(varchar,@fiDt,23)

set @ffev=CONVERT(varchar,@ffDt,23)
 ...
 --Par_ProcesosInternosDetallado '','','','','','','','','','2023-03-08','2023-03-08'
...
create table #TEMRPTUDP (
	...
	mermatotalGeneral decimal (17,2)
	,desperdiciototal decimal (17,2)
	,totalliq decimal(17,2)
	,totalmp decimal (17,2)
	--select *from productionlot where id =1041602
...
,mermatotalGeneral = (select sum(totalQuantityLoss) from productionlot plss where plss.id in (select id from #Lotetemp))
,isnull(case when despmu.id = 4 then
desp.quantity else 
desp.quantity * 2.2046
end,0) as DespLibrasDetalle
,totalliq = (select sum(totalQuantityLiquidation) from productionlot plss where plss.id in (select id from #Lotetemp))
,totalmp = (select sum(totalQuantityRecived) from productionlot plss where plss.id in (select id from #Lotetemp))
--,totalliq =  (select sum(totalQuantityLiquidation) from productionlot plss where CONVERT(varchar,plss.receptionDate ,23)between CONVERT(varchar,@fiDt,23) and CONVERT(varchar,@ffDt,23))
--,totalmp = (select sum(totalQuantityRecived) from productionlot plss where   CONVERT(varchar,plss.receptionDate ,23)between CONVERT(varchar,@fiDt,23) and CONVERT(varchar,@ffDt,23))
...
--tmp.quantitytotal 
tmp.quantityPoundsLiquidation
when PRES2.id_metricUnit = 1 then
tmp.quantitytotal 
...
tmp.quantityPoundsLiquidation
when PRES2.id_metricUnit = 1 then
  tmp.quantitytotal  / 2.2046 else tmp.quantityPoundsLiquidation
--tmp.quantitytotal -- 
end as LibrasDetalle*/
...
select
	...
	--ROUND(isnull(case when MPROLIQ = 'MERMA'then sum(tmpquantityPoundsLiquidationliqlibrastotalrecobidas) end,0),2) as sumamerma,
...
CODIGO != 'REC'  
AND Estado <> 'ANULADO'
--AND PRODLnumberNumerodeLote = 'RLB000000194'
...
/*
	EXEC spPar_ProcesoInternoDetallado @id=N'',@estado=0,@nlote=N'',@ninterno=N'',
	@unidad=N'',@proceso=N'',@producto=N'',@bodega=N'',@ubicacion=N'',@fi=N'2022/02/01',@ff=N'2022/02/28'
*/
GO