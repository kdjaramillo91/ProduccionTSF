/****** Object:  StoredProcedure [dbo].[SP_ResumenLiquidacionesProveedor]    Script Date: 20/03/2023 05:43:31 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- EXEC [SP_ResumenLiquidacionesProveedor] NULL,'20221001','20221015'
CREATE OR ALTER PROC [dbo].[spPar_ResumenLiquidacionProveedor]
	@proveedor int,
	@fi varchar(10) = '',
	@ff varchar(10) = '',
	...
	END
	--and year(pl.receptionDate)=2021 and month(pl.receptionDate) in (10)
	Exec(@cad) 
	...
	Select	
			...
			SUM([Total Liquidación])/NULLIF(SUM([Libras Procesadas]),0) as 'PrecioTotal'
		
		from	#ResumenLiquidacionSP
		GROUP BY [etiquetaComprador],[proveedor]
		ORDER BY [proveedor]
GO
