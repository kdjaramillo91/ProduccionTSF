...
GO
/****** Object:  StoredProcedure [dbo].[par_GuideRemisionViaticoGeneral]    Script Date: 21/02/2023 01:50:47 p. m. ******/
SET ANSI_NULLS ON
GO
...
Create procedure [dbo].[spPar_GuideRemisionViaticoTerrestre]
...
select RG.id  as RG_id ,
    	logo = Company.logo2,
		documento = convert(varchar(250),Document.sequential),
		...
		, drgcv.[sequential] as [NumeroViaticoPersonal]	
		, perUser.fullname_businessName as [UsuarioPago]	 
from RemissionGuide RG
inner join [dbo].[RemissionGuideCustomizedViaticPersonalAssigned] rgcvpa on rg.[id] = rgcvpa.[id_RemissionGuide]
inner join [dbo].[Document] drgcv on rgcvpa.[id_ViaticPersonalAssigned] = drgcv.[id]
inner join Person Person  on Person.id =RG.id_providerRemisionGuide  
inner join  RemissionGuideAssignedStaff   RGAS on RGAS.id_remissionGuide = rg.id
...
inner join  [dbo].[tbsysCatalogState] tcs on rgcvpa.[id_paymentstate] = tcs.[id] and tcs.[codeClasification] = '01'
left outer join [dbo].[User] userap on rgcvpa.id_UserApproved = userap.id
...
order by Document.sequential desc

	   --execute par_GuideRemisionViaticoGeneral
