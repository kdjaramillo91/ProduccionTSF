...
{
    internal class GuiaRemisionPersAuxiliar
    {
        private const string m_StoredProcedureName = "spPar_GuiaRemisionPers";
        private const string m_StoredProcedureNameSub = "spPar_GuiaRemisionMatDesp";
        ...
        {
            var rptGuiaRemisionPers = new GuiaRemisionPersDataSet();
            ...
            using (SqlConnection db = sqlConnection)
            {
                ...
                var detailsResult = db.Query<GuiaRemisionPersModel>(m_StoredProcedureName, queryParameters, commandType: CommandType.StoredProcedure)
                    .ToList();

                foreach (var detailResult in detailsResult)
                {
                    var cabeceraRow = rptGuiaRemisionPers.CabeceraGuiaRemisionPers.NewCabeceraGuiaRemisionPersRow();
                    ...
                    cabeceraRow.ListaPrecio = detailResult.ListaPrecio;
                    cabeceraRow.Camaronera = detailResult.Camaronera;

                    rptGuiaRemisionPers.CabeceraGuiaRemisionPers.AddCabeceraGuiaRemisionPersRow(cabeceraRow);
                }
        ...
        {
            using (var report = new RptGuiaRemisionPers())
            {
