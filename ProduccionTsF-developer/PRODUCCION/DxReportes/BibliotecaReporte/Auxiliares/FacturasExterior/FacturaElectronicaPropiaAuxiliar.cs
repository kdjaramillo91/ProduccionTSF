...
{
   internal class FacturaElectronicaPropiaAuxiliar
    {
        ...
        private static FacturaElectronicaPropiaDataSet PrepareData(SqlConnection sqlConnection, Parametro[] parametros)
        {
            var rptFacturaElectroniaPropia = new FacturaElectronicaPropiaDataSet();
            ...
            using (SqlConnection db = sqlConnection)
            {
                ...
                var detailsResult = db.Query<FacturaElectronicaPropiaModel>(m_StoredProcedureName, queryParameters, commandType: CommandType.StoredProcedure)
                    .ToList();

                foreach (var detailResult in detailsResult)
                {
                    var cabeceraRow = rptFacturaElectroniaPropia.CabeceraFacturaElectronicaPropia.NewCabeceraFacturaElectronicaPropiaRow();
                    ...
                    rptFacturaElectroniaPropia.CabeceraFacturaElectronicaPropia.AddCabeceraFacturaElectronicaPropiaRow(cabeceraRow);
                }

                if (db.State == ConnectionState.Open) { db.Close(); }
            }

            return rptFacturaElectroniaPropia;
        }

       private static Stream SetDataReport(FacturaElectronicaPropiaDataSet rptfacturaElectronicaPropiaDataSet)
        {
            using (var report = new RptFacturaElectronicaPropia())
            {
                report.SetDataSource(rptfacturaElectronicaPropiaDataSet);
