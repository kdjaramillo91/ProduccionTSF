...
function OnGridViewLiquidationTransferDetailsInit(s, e) {
    UpdateTitlePanelLiquidationDetailsTransfer();
}
...
function UpdateTitlePanelLiquidationDetailsTransfer() {
    var selectedFilteredRowCount = GetSelectedFilteredRowCountLiquidationTransferDetails();

    var text = "Total de elementos seleccionados: <b>" + gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedRowCount() + "</b>";
    var hiddenSelectedRowCount = gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedRowCount() - GetSelectedFilteredRowCountLiquidationTransferDetails();
    if (hiddenSelectedRowCount > 0)
        text += "Elementos ocultos por filtrado seleccionados: <b>" + hiddenSelectedRowCount + "</b>";
    text += "<br />";
    ...
    if ($("#selectAllMode").val() !== "AllPages") {
        SetElementVisibility("lnkSelectAllRowsLiquidations", gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedRowCount() > 0 && gvProductionLotEditFormProductionLotLiquidationsTransferDetail.cpVisibleRowCount > selectedFilteredRowCount);
        SetElementVisibility("lnkClearSelectionLiquidations", gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedRowCount() > 0);
    }

    btnRemoveLiquidation.SetEnabled(gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedRowCount() > 0);
}
...
function GetSelectedFilteredRowCountLiquidationTransferDetails() {
    return gvProductionLotEditFormProductionLotLiquidationsTransferDetail.cpFilteredRowCountWithoutPage +
        gvProductionLotEditFormProductionLotLiquidationsTransferDetail.GetSelectedKeysOnPage().length;
}
...
function OnGridViewLiquidationsDetailTransferSelectionChanged(s, e) {
    //UpdateTitlePanelLiquidationDetailsTransfer();
    s.GetSelectedFieldValues("id_item", GetSelectedFieldValuesCallbackLiquidationsDetail);
...
function OnGridViewLiquidationDetailsTransferBeginCallback(s, e) {
    customCommand = e.command;
    e.customArgs["Act"] = false;
    e.customArgs["id_wareHouse"] = id_wareHouseTransfer.GetValue();
}
...
function OnGridViewLiquidationDetailsTransferEndCallback(s, e) {
    //UpdateTitlePanelLiquidationDetailsTransfer();

    //UpdateProductionLotTotals();
    gvProductionLotEditFormProductionLotLiquidationsTransferDetail.PerformCallback();
}
...
function OnItemDetailBeginCallback(s, e) {
    ...
    if (s !== undefined) {
        ...
        var itemCurrent = 0
        if (id_itemInit == null) {
            itemCurrent = $("#ItemDetail" + gvProductionLotEditFormProductionLotLiquidationsDetail.cpRowKey).val();
        }
        else
        {
            itemCurrent = id_itemInit;
        }
            
        e.customArgs["id_item"] = itemCurrent;
        e.customArgs["id_itemType"] = ComboBoxItemType.GetValue() === undefined ? null : ComboBoxItemType.GetValue();
        e.customArgs["id_size"] = ComboBoxSize.GetValue() === undefined ? null : ComboBoxSize.GetValue();
        e.customArgs["id_trademark"] = ComboBoxTrademark.GetValue() === undefined ? null : ComboBoxTrademark.GetValue();
        e.customArgs["id_presentation"] = ComboBoxPresentation.GetValue() === undefined ? null : ComboBoxPresentation.GetValue();
