...
function Update(approve) {
    ...
    if (valid && validT && valid2) {
        ...
        var carRegistrationThirdTmp = carRegistrationThird.GetText();
        var driverNameThirdTmp = driverNameThird.GetText();
        var SailEntranceTmp = SailEntranceThird.GetText();
        var GuideNumberTmp = GuideNumber.GetText();

        var id = document.getElementById("id_remissionGuide").getAttribute("idremissionGuide");

        var data = "id=" + id + "&" + "approve=" + approve + "&"
                            + "entranceTimeBuilding=" + entranceTimePlantProductionTmp
                            + "&carRegistrationThird=" + carRegistrationThirdTmp
                            + "&driverNameThird=" + driverNameThirdTmp
                            + "&sailEntrance=" + SailEntranceTmp
                            + "&guideNumber=" + GuideNumberTmp
                            + "&" + $("#formEditRemissionGuideControlVehicleEntranceThird").serialize();
        var url = (id === "0") ? "RemissionGuideControlVehicleEntranceThird/RemissionGuideControlVehicleEntranceThirdPartialAddNew"
                               : "RemissionGuideControlVehicleEntranceThird/RemissionGuideControlVehicleEntranceThirdPartialUpdate";
...
function ButtonCancel_Click(s, e) {
    ...
    showPage("RemissionGuideControlVehicleEntranceThird/Index", null);
}
...
function Remove(s, e) {
    ...
    activeGridView.GetSelectedFieldValues(key, function (values) {
        ...
        var url = "RemissionGuideControlVehicleEntranceThird/DeleteSelectedRemissionGuideControlVehicleEntranceThirdDetails";
...
function UpdatePagination() {
    var current_page = 1;
    $.ajax({
        url: "RemissionGuideControlVehicleEntranceThird/InitializePagination",
        type: "post",
