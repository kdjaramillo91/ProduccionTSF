// Botones de acción
function AddNewUserObject(s, e) {
	gvUserObjects.AddNewRow();
}
...
// #region Permisos de Objetos

function onObjectControllerValidate(s, e) {
	if (s.GetValue() === null) {
		e.isValid = false;
		e.errorText = "Campo Obligatorio";
	}
}
function onObjectPermissionValidate(s, e) {
	if (s.GetValue() === null) {
		e.isValid = false;
		e.errorText = "Campo Obligatorio";
	}
...
function UserObjectOnBeginCallBack(s, e) {
	e.customArgs['id_user'] = $("#id_user").val();
	e.customArgs['id'] = gvUserObjects.cpRowKey == undefined ? 0 : gvUserObjects.cpRowKey;
}
