function Update(approve) {
    ...
    var valid = true;
    var validDocumentReceptionDispatchMaterials = ASPxClientEdit.ValidateEditorsInContainerById("documentReceptionDispatchMaterials", null, true);



    if (validDocumentReceptionDispatchMaterials) {
    ...
    if (/*gvRemissionGuideDispatchMaterialEditForm.cpRowsCount === 0 ||*/ gvRemissionGuideDispatchMaterialEditForm.IsEditing()) {
        UpdateTabImage({ isValid: false }, "tabReceptionDispatchMaterialsDetails");
        valid = false;
    } else {
        UpdateTabImage({ isValid: true }, "tabReceptionDispatchMaterialsDetails");
    }

    if (/*gvReceptionDispatchMaterialsDetailEditForm.cpRowsCount === 0 ||*/ gvReceptionDispatchMaterialsDetailEditForm.IsEditing()) {
        UpdateTabImage({ isValid: false }, "tabReceptionDispatchMaterialsDetails");
        valid = false;
    } else {
        UpdateTabImage({ isValid: true }, "tabReceptionDispatchMaterialsDetails");
    ...
    //if (codeState == "01") {
    //if (gvInvoiceCommercialEditFormDetail.cpRowsCount === 0 ||
    //    gvInvoiceCommercialEditFormDetail.IsEditing()) {
    ...
    if (valid) {
        var id = $("#id_receptionDispatchMaterials").val();

        var data = "id=" + id + "&" + "approve=" + approve + "&" + $("#formEditReceptionDispatchMaterials").serialize();

        var url = (id === "0") ? "ReceptionDispatchMaterials/ReceptionDispatchMaterialsAddNew"
                               : "ReceptionDispatchMaterials/ReceptionDispatchMaterialsUpdate";
...
function ButtonCancel_Click(s, e) {
    showPage("ReceptionDispatchMaterials/Index", null);
}
...
function ApproveDocument(s, e) {
    ...
    showConfirmationDialog(function () {
        Update(true);
    }, "¿Desea aprobar la Recepción de Materiales?");
...
function CancelDocument(s, e) {
    showConfirmationDialog(function () {
        var data = {
            id: $("#id_receptionDispatchMaterials").val()
        };
        showForm("ReceptionDispatchMaterials/Cancel", data);
    }, "¿Desea anular la Recepción de Materiales?");
}
...
function RevertDocument(s, e) {
    showConfirmationDialog(function () {
        ...
        showForm("ReceptionDispatchMaterials/Revert", data);
    }, "¿Desea reversar la Recepción de Materiales?");
}
...
//    //Refresh(s, e);
//    $.ajax({
//        url: "InvoiceCommercial/UpdateDetail",
//        type: "post",
...
function UpdateView() {
    var id = parseInt($("#id_receptionDispatchMaterials").val());
    ...
    $.ajax({
        url: "ReceptionDispatchMaterials/Actions",
        type: "post",
        data: { id: id },
        async: true,
        cache: false,
