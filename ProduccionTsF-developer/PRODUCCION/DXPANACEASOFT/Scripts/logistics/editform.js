...
function Update(approve) {
    ...
    var validDocumentCut = ASPxClientEdit.ValidateEditorsInContainerById("tableTabDocument", null, true);
    ...
    if (typeof( gvDispatchMaterials) != "undefined" )
    {
        var _enEdicion = gvDispatchMaterials.batchEditApi.HasChanges();
        if (_enEdicion ) {
    ...
    if (typeof (gvReceptionDispatchMaterialsDetailEditForm) != "undefined")
    {
        if (gvReceptionDispatchMaterialsDetailEditForm.IsEditing()) {
            UpdateTabImage({ isValid: false }, "tabDispatchMaterials");
            valid &= false;
        }
    }
    ...
    if (gvSecuritySeals.IsEditing()) {
        UpdateTabImage({ isValid: false }, "tabSecuritySeals");
        valid &= false;
    }

    if (gvAssignedStaff.IsEditing()) {
    ...
    if (valid) {
        ...
        var url = (id === 0 || id === "0") ? "Logistics/RemissionGuidePartialAddNew" : "Logistics/RemissionGuidePartialUpdate";
...
function ButtonCancel_Click(s, e) {
    $("#GridMessageErrorLogistics").hide();
...
function CopyDocument(s, e) {
    showPage("Logistics/RemissionGuideCopy", { id: parseInt(document.getElementById("id_remissionGuide").getAttribute("idremissionGuide")) });
}
...
function ApproveDocument(s, e) {
    ...
    $.ajax({
        url: "Logistics/RemissionGuidePartialUpdate",
        type: "post",
        data: data,
        async: false,
        cache: false,
...
function CheckAutorizeRSIDocument(s, e) {
    showConfirmationDialog(function () {
        ...
        showForm("Logistics/CheckAutorizeRSI", data);
...
function RevertDocument(s, e) {
    ...
    showConfirmationDialog(function () {
        ...
        showForm("Logistics/RevertLV", data);
    }, "¿Desea reversar la guía de remisión?");
}
...
function PrintRide(s, e) {
    ...
    $.ajax({
        url: 'Logistics/PrintRemissionGuideRideReport',
        data: data,
        async: true,
        cache: false,
        type: 'POST',
...
function OnidentificationCard(s, e) {
    var answerControlsEnabledTmp = $("#answerControlsEnabled").val();
    if (answerControlsEnabledTmp === "NO") {
        driverNameThird.SetEnabled(false);
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result !== null) {
                    ...
                    if (result.id_providerTBilling !== null) {
                        id_VehicleProviderTranportistBilling.SetValue(result.id_providerTBilling);
                    } else {
                        id_VehicleProviderTranportistBilling.SetValue(null);
                    }
                }
            },
            complete: function () {
...
//AUXILIAR FUNCTIONS
function UpdateButtonsEditForm(result) {
    if (result !== undefined) {
         
        document.getElementById("id_remissionGuide").setAttribute("idremissionGuide", result.idEntity);
        document.getElementById("codeDocumentState").value = result.codeStateAnswer;
        documentState.SetValue(result.nameStateAnswer);

        numberDoc.SetValue(result.strNumberDocAnswer);

        if (result.btnOnEditFormRemissionGuide !== undefined) {
            btnApprove.SetEnabled(result.btnOnEditFormRemissionGuide.btnApprove);
            btnAutorize.SetEnabled(result.btnOnEditFormRemissionGuide.btnAutorize);
            btnProtect.SetEnabled(result.btnOnEditFormRemissionGuide.btnProtect);
            btnCancel.SetEnabled(result.btnOnEditFormRemissionGuide.btnCancel);
            btnRevert.SetEnabled(result.btnOnEditFormRemissionGuide.btnRevert);
            btnreassignment.SetEnabled(result.btnOnEditFormRemissionGuide.btnreassignment);
            btnCancelRG.SetEnabled(result.btnOnEditFormRemissionGuide.btnCancelRG);
            btnPrint.SetEnabled(result.btnOnEditFormRemissionGuide.btnPrint);
            btnPrintAlldoc.SetEnabled(result.btnOnEditFormRemissionGuide.btnPrintAlldoc);
            btnPrintManual.SetEnabled(result.btnOnEditFormRemissionGuide.btnPrintManual);
            btnPrintAlldocManual.SetEnabled(result.btnOnEditFormRemissionGuide.btnPrintAlldocManual);
            btnUpdate.SetEnabled(result.btnOnEditFormRemissionGuide.btnSave);
        }

        if (result.messageAnswer !== null && result.messageAnswer !== undefined) {
            gridMessageErrorLogistics.SetText(result.messageAnswer);
            $("#GridMessageErrorLogistics").show();
        }
...
function OnIsCopackinRG_CheckedChanged(s, e) {
    id_productionUnitProvider.ClearItems();
     
    if (isCopackingRG.GetChecked()) {
        isOwn.SetValue(true);
        isCopakingRGProvider(true);
    }
    else {
        isOwn.SetValue(false);
        isCopakingRGProvider(false);
    }

}


function isCopakingRGProvider(isCopacking) {
    id_providerRemisionGuide.ClearItems();
    if (isCopacking) {
        $.ajax({
            url: "Logistics/ProviderCopacking",
            type: "post",
            async: true,
            cache: false,
            error: function (error) {
            ...
            success: function (result) {
                for (var i = 0; i < result.length; i++) {
                    ...
                    id_providerRemisionGuide.AddItem(itemType.id, itemType.name);
                }
            },
            complete: function () {
            }
