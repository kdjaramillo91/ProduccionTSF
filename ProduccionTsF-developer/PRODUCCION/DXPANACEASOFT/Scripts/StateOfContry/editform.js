...
function Update(approve) {
    var valid = ASPxClientEdit.ValidateEditorsInContainer(null, "tabdetail", true);
     ...
     if (valid) {
         ...
         var id = $("#id_StateOfContry").val();
   
         
      
         var data = "id=" + id + "&" + "approve=" + approve + "&" + $("#FormEditStateOfContry").serialize();
         var url = (id === "0") ? "StateOfContry/StateOfContryPartialAddNew"
                               : "StateOfContry/StateOfContryPartialUpdate";
...
function CancelDocument(s, e) {
    showConfirmationDialog(function () {
        var data = {
            id: $("#id_StateOfContry").val()
        };
        showForm("StateOfContry/Cancel", data);
...
function RevertDocument(s, e) {
    showConfirmationDialog(function () {
        ...
        showForm("StateOfContry/Revert", data);
    }, "¿Desea Activar?");
}
...
function UpdateView() {
    //var id = parseInt($("#id_StateOfContry").val());
