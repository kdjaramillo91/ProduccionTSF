...
function OnQuantityPackingMaterialValidation(s, e) {
    if (s.GetValue() === null) {
        ...
        var quantityRequiredForProductionLotAux = gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.cpEditingRowQuantityRequiredForProductionLot;
...
function ItemPackingMaterialCombo_OnInit(s, e) {
    ...
    $.ajax({
        url: "ProductionLotReception/PackingMaterialDetails",
        type: "post",
        data: data,
        async: false,
        cache: false,
...
function DetailsUpdateItemPackingMaterialsInfo(data) {
    ...
    if (id_itemPackingMaterial != null) {

        $.ajax({
            url: "ProductionLotReception/ItemPackingMaterialDetailsData",
            type: "post",
            data: data,
            async: true,
            cache: false,
...
function UpdateTitlePanelPackingMaterialDetails() {
    ...
    var text = "Total de elementos seleccionados: <b>" + gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedRowCount() + "</b>";
    var hiddenSelectedRowCount = gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedRowCount() - GetSelectedFilteredRowCountPackingMaterialDetails();
    ...
    if ($("#selectAllMode").val() !== "AllPages") {
        SetElementVisibility("lnkSelectAllRowsPackingMaterials", gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedRowCount() > 0 && gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.cpVisibleRowCount > selectedFilteredRowCount);
        SetElementVisibility("lnkClearSelectionPackingMaterials", gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedRowCount() > 0);
    }

    btnRemovePackingMaterial.SetEnabled(gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedRowCount() > 0);
}

function GetSelectedFilteredRowCountPackingMaterialDetails() {
    return gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.cpFilteredRowCountWithoutPage +
           gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.GetSelectedKeysOnPage().length;
}
...
function gvEditPackingMaterialDetailsClearSelection() {
    gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.UnselectRows();
}

function gvEditPackingMaterialDetailsSelectAllRows() {
    gvProductionLotReceptionEditFormProductionLotPackingMaterialsDetail.SelectRows();
}
