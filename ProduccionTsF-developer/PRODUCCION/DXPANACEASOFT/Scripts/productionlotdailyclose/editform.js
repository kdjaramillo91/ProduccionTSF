function Update(approve) {
    ...
    //if (codeState == "01") {
    if (gvProductionLotDailyCloseEditFormProductionLotsDetail.cpRowsCount === 0 ||
        gvProductionLotDailyCloseEditFormProductionLotsDetail.IsEditing()) {
        UpdateTabImage({ isValid: false }, "tabProductionLotsDetails");
        valid = false;
    }
    //}

    if (valid) {
        var id = $("#id_productionLotDailyClose").val();

        var data = "id=" + id + "&" + "approve=" + approve + "&" + $("#formEditProductionLotDailyClose").serialize();

        var url = (id === "0") ? "ProductionLotDailyClose/ProductionLotDailyClosesAddNew"
                               : "ProductionLotDailyClose/ProductionLotDailyClosesUpdate";
...
function ButtonUpdateClose_Click(s, e) {
    ...
    if (valid) {
        ...
        var data = "id=" + id + "&" + $("#formEditProductionLotDailyClose").serialize();
...
function CancelDocument(s, e) {
    showConfirmationDialog(function () {
        ...
        showForm("ProductionLotDailyClose/Cancel", data);
    }, "¿Desea anular el cierre?");
}
...
function RevertDocument(s, e) {
    showConfirmationDialog(function () {
        ...
        showForm("ProductionLotDailyClose/Revert", data);
    }, "¿Desea reversar el cierre?");
}
...
function PrintDocument(s, e) {
    ...
    $.ajax({
        ...
        type: "post",
        data: { id: $("#id_ProductionLotDailyClose").val() },
        async: true,
        cache: false,
        error: function (error) {
            console.log(error);
...
function RefreshDetail(s, e) {
    //Refresh(s, e);
    $.ajax({
        url: "ProductionLotDailyClose/UpdateDetail",
        type: "post",
        data: null,//{ id: $("#id_ProductionLotDailyClose").val() },
        async: true,
        cache: false,
        error: function (error) {
            console.log(error);
        ...
        success: function (result) {
            ...
            gvProductionLotDailyCloseEditFormProductionLotsDetail.PerformCallback();
        },
        complete: function () {
            hideLoading();
        }
...
function UpdateView() {
    var id = parseInt($("#id_productionLotDailyClose").val());
    ...
    $.ajax({
        url: "ProductionLotDailyClose/Actions",
        type: "post",
        data: { id: id },
        async: true,
        cache: false,
...
function UpdatePagination() {
    var current_page = 1;
    $.ajax({
        url: "ProductionLotDailyClose/InitializePagination",
        type: "post",
        data: { id_productionLotDailyClose: $("#id_productionLotDailyClose").val() },
        async: false,
        cache: false,
        error: function (error) {
            console.log(error);
