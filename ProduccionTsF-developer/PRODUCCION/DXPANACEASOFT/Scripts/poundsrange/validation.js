function OnPoundsRangeNameValidation(s, e) {
    if (e.value === null) {
        e.isValid = false;
...
function OnCodePoundsRangeValidation(s, e) {
    if (e.value === null) {
        e.isValid = false;
        e.errorText = "Campo Obligatorio";
    } else {
        if (e.value.length > 21) {
            ...
            $.ajax({
                url: "PoundsRange/ValidateCodePoundsRange",
                type: "post",
                async: false,
                cache: false, data: {
                    id_PoundsRange: gvPoundsRange.cpEditingRowKey,
                    code: e.value
                },
