...
function AprovedItem() {
	...
	var result = DevExpress.ui.dialog.confirm("Desea Aprobar el Descabezado Seleccionado?", "Confirmar");
...
function ReverseItem() {
    ...
    var result = DevExpress.ui.dialog.confirm("Desea Reversar el Descabezado Seleccionado?", "Confirmar");
	result.done(function (dialogResult) {
		if (dialogResult) {
			$.ajax({
...
function AnnulItem() {
    ...
    var result = DevExpress.ui.dialog.confirm("Desea Anular el Descabezado Seleccionado?", "Confirmar");
	result.done(function (dialogResult) {
		if (dialogResult) {
			$.ajax({
				url: 'Headless/Annul',
...
function RefreshGrid() {
    ...
    var data = {
        ...
        numberLotOrigin: TextBoxNumberLotOrigin.GetText()
    };

	showPartialPage($("#grid"), 'Headless/SearchResult', data);
}


function PrintItemGeneral() {
	var codeReport = "DESCA";
	var data = "codeReport=" + codeReport + "&" + $("#HeadlessFilterForm").serialize();
	// 
	if (data != null) {
		$.ajax({
			url: "Headless/PrintReportGeneral",
			type: "post",
			data: data,
			async: true,
			cache: false,
