...
{
	public static class DataProviderAllocationCostPeriod
	{
		...
		public static IEnumerable<ProductionCostCoefficient> ProductionCoefficients(
			int? id_executionType, bool accountingValue, int? id_coefficient)
		{
			var coefficientsResult = new DBContext()
				.ProductionCostCoefficient
				.Where(c => (c.isActive && (c.id_executionType == id_executionType)) || (c.id == id_coefficient))
				.ToArray()
				.AsEnumerable();

			if (accountingValue)
			{
				coefficientsResult = coefficientsResult
					.Where(c => !String.IsNullOrWhiteSpace(c.id_planDeCuentas) || (c.id == id_coefficient));
			}
			else
			{
				coefficientsResult = coefficientsResult
		...
		public static Person GetPersonById(int? id_person)
		{
			return new DBContext()
				.Person
				.FirstOrDefault(p => p.id == id_person);
		}
	}
}