...
namespace DXPANACEASOFT.DataProviders
{
    public class DataProviderCountry
    {
       private static DBContext db = null;
        ...
        public static IEnumerable ContriesCities(int id_company)
        {
            db = new DBContext();
            var model = db.City.Where(r => r.isActive && r.Country.id_company == id_company).Select(s =>
                new { nombreCiudad = s.name,
                      s.code, 
                      s.id,
                      nombrePais=s.Country.name
                    }
                ).OrderBy(o=> o.nombrePais ).ThenBy( o2=> o2.nombreCiudad).ToList();

            return model;


        }
        public static IEnumerable AllcountryType()
        {
            db = new DBContext();
