...
{
    internal class ItemGroupCategoryExcelFileParser : ExcelFileParserBase
    {
        private const string m_TemplateRelativePath = "~/App_Data/Templates/ItemGroupCategoryTemplate.xlsx";

        private static readonly string m_TemplateAbsolutePath;

        static ItemGroupCategoryExcelFileParser()
        {
            m_TemplateAbsolutePath = HttpContext.Current.Server.MapPath(m_TemplateRelativePath);
        }
        internal ItemGroupCategoryExcelFileParser(string contentFilePath, string contentFileMime)
            : base(contentFilePath, contentFileMime, true)
        {
        }
        private ItemGroupCategoryExcelFileParser(string contentFilePath, string contentFileMime, bool readOnly)
            : base(contentFilePath, contentFileMime, readOnly)
        {
        }
        internal ImportResult ParseItemGroupCategory(int activeCompanyId, User activeUser)
        {
            ...
            var importResult = ProcesarItemGroupCategory(dataTables[0], activeCompanyId, activeUser);

            return importResult;
        }
        private static string GetTablaItemTypeColumnNames()
        {
            string columnTypes = $"CODIGO,NOMBRE,ID_SUBGRUPO,DESCRIPCION";
            return columnTypes;
        }

        private ImportResult ProcesarItemGroupCategory(DataTable dtCabecera, int activeCompanyId, User activeUser)
        {
            ...
            var itemsCrear = new List<ItemGroupCategory>();
            ...
            foreach (DataRow drCabecera in dtCabecera.Rows)
            {
                ...
                try
                {
                    ...
                    {
                        var itemTypeCategoriaSubir = new ItemGroupCategory()
                        {
                        ...
                        try
                        {
                            var itemTypeItemTypeCategories = new ItemGroupItemGroupCategory()
                            {
                                id_itemGroup = drCabecera.GetInteger("ID_SUBGRUPO"),
                                id_itemGroupCategory = itemTypeCategoriaSubir.id,
                            };

                            itemTypeCategoriaSubir.ItemGroupItemGroupCategory.Add(itemTypeItemTypeCategories);

                            itemsCrear.Add(itemTypeCategoriaSubir);
                        }
                        catch (Exception)
                        {
                            throw new Exception("El Subgrupo no puede estar vacio.");                           
                        }                     
                    }
                    else
                    {
            ...
            for (int i = 0; i < itemsCrear.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (itemTypeCategoryCrear.ItemGroupItemGroupCategory.Count <= 0)
                    {
            ...
            using (DbContextTransaction trans = db.Database.BeginTransaction())
            {
                try
                {
                    db.ItemGroupCategory.AddRange(itemsCrear);
        ...
        #region Métodos de descarga de plantilla
        internal static FileContentResult GetTemplateFileContentResult(int empresa)
        {
            ...
            var excelParser = new ItemGroupCategoryExcelFileParser(contentPath, ExcelXlsmMime, false);
            ...
            try
            {
                fileName = Path.ChangeExtension(
                    $"CategoríaDeProducto_{empresa}_{DateTime.Now:yyyyMMdd HHmm}",
        ...
        private static void CopyTipoProductoData(int empresa, OleDbConnection cnn)
        {
            ...
            var tipoProductos = db.ItemGroup
            ...
            if (tipoProductos.Count() > 0)
            {
                using (var cmd = cnn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "INSERT INTO [DATA_CATEGORIA2$] ([NOMBRE], [ID], [CODIGO]) VALUES (?, ?, ?)";
                    ...
                    {
                        //var GrupoCategoria = db.ItemGroupCategory.FirstOrDefault(t => t.id == tipoProducto.id);
                        // Insertamos los valores en la tabla de Excel
                        //codigoLinea.SetStringValue(GrupoCategoria.code);
